...
GlobalDataset_<-read.csv('input_to_update/GlobalDataset.csv') %>% 
  mutate(DateReport1=as.Date(DateReport1)) %>% 
  mutate(ADM0NAME=if_else(ADM0NAME=='Kosovo','Kosovo(1)',ADM0NAME)) %>% 
  arrange(ADM0NAME) %>% 
...
PlotTimeLine<-function(ctry,CasesOrDeaths,StartDate,EndDate,Log,Measure){
  ...
  Dataset<-CountryDataset %>% 
    pivot_longer(cols=c(GlobalIndex:PublicTransport)) %>% 
  ...
  plot_timeline<-ggplot(Dataset,aes(x=DateReport1,y=Height))+
    ...
    geom_tile(stat='identity',aes(x=DateReport1,y=Height,alpha=value,fill=factor(name,levels=c('Masks',"Schools",'Businesses','Gatherings','Movements','Borders','PublicTransport',"GlobalIndex"))),position='stack')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 50, label = "PHSM Severity Index",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 125, label = "PublicTransport",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 175, label = "International Travel",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 225, label = "Movements",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 275, label = "Gatherings",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 325, label = "Businesses",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 375, label = "Schools",color='white')+
    annotate("text", x = StartDate+(EndDate-StartDate)*1.9/3, y = 425, label = "Masks",color='white')+
    annotate('text', x = EndDate, y=50, label = 'PHSM Severity Index', color='#04319D',hjust = 0)+
    annotate("text", x = EndDate, y = 125, label = "Public Transports",color='#324836',hjust=0)+
    annotate("text", x = EndDate, y = 175, label = "International Travel",color='#AD0C00',hjust=0)+
    annotate("text", x = EndDate, y = 225, label = "Movements",color='#0A698A',hjust=0)+
    annotate("text", x = EndDate, y = 275, label = "Gatherings",color='#A1078B',hjust=0)+
    annotate("text", x = EndDate, y = 325, label = "Businesses",color='#3C8E05',hjust=0)+
    annotate("text", x = EndDate, y = 375, label = "Schools",color='#D0C600',hjust=0)+
    annotate("text", x = EndDate, y = 425, label = "Masks",color='#E57E00',hjust=0)+
  scale_fill_manual(breaks=c('Masks',"Schools",'Businesses','Gatherings','Movements','Borders','PublicTransport',"GlobalIndex"),
                    labels=c('Masks','Schools measures', 'Workplace measures','Restrictions on gatherings','Stay-at-home requirements', 'International travel restrictions','Public Transports','Global Severity Index'),
                    values=c('#E57E00','#D0C600','#3C8E05','#A1078B','#0A698A','#AD0C00','#324836','#04319D'))+
  scale_x_date(date_breaks = "15 days",date_labels =  "%d-%b",limits=c(StartDate,EndDate))+
  ...
  if (Measure!='All measures together'){
    ...
    clr<-case_when(Measure=='Schools'~ '#D0C600',
                    ...
                    Measure=='PublicTransport' ~ '#324836')
...
TrickLegend<-ggplot(LegendTimeLine)+
  geom_tile(stat='identity',aes(x=x,y=Height,alpha=Alpha,fill=factor(Index,levels=c('Masks',"Schools",'Businesses','Gatherings','Movements','Borders','PublicTransport',"GlobalIndex"))),position='stack')+
  scale_fill_manual(breaks=c('Masks','Schools','Businesses','Gatherings','Movements','Borders','PublicTransport','GlobalIndex'),
                    labels=c('Masks','Schools measures', 'Workplace measures', 'Restrictions on gatherings', 'Stay-at-home requirements', 'International travel restrictions','Restrictions on public transports','Global Severity Index'),
                    values=c('#E57E00','#D0C600','#3C8E05','#A1078B','#0A698A','#AD0C00','#324836','#04319D'))+
  theme_void()+
