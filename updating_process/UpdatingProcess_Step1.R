...
folderInput_1<-paste0(folder_,'/input_step1')
folderInput_2<-paste0(folder_,'/input_step2')
folderOutput<-paste0(folder_,'/output_step1')



readSeverityDataset <- function(country) {
  df <- read_excel(paste0(folderInput_1,"/SeverityIndex.xlsx"), sheet = country,col_names=FALSE) #DataFrame
...
ListCountries_Index<-excel_sheets(paste0(folderInput_1,"/SeverityIndex.xlsx"))
...
ExistingKeyDates_PHSM<-read_excel(paste0(folderInput_2,"/KeyDates.xlsx"),sheet='Severity Index') %>% mutate(Date=as.Date(Date))
ExistingKeyDates_Masks<-read_excel(paste0(folderInput_2,"/KeyDates.xlsx"),sheet='Masks') %>% mutate(Date=as.Date(Date),ADM0NAME=as.character(ADM0NAME))
ExistingKeyDates_Schools<-read_excel(paste0(folderInput_2,"/KeyDates.xlsx"),sheet='Schools') %>% mutate(Date=as.Date(Date),ADM0NAME=as.character(ADM0NAME))
ExistingKeyDates_Businesses<-read_excel(paste0(folderInput_2,"/KeyDates.xlsx"),sheet='Businesses') %>% mutate(Date=as.Date(Date),ADM0NAME=as.character(ADM0NAME))
